...
-export_records(['diameter_relay_Proxy-Info',
		 'diameter_relay_Failed-AVP',
		 'diameter_relay_Experimental-Result',
		 'diameter_relay_Vendor-Specific-Application-Id',
		 'diameter_relay_E2E-Sequence']).

-record('diameter_relay_Proxy-Info',
	{'Proxy-Host', 'Proxy-State', 'AVP' = []}).

-record('diameter_relay_Failed-AVP', {'AVP' = []}).

-record('diameter_relay_Experimental-Result',
	{'Vendor-Id', 'Experimental-Result-Code'}).

-record('diameter_relay_Vendor-Specific-Application-Id',
	{'Vendor-Id' = [], 'Auth-Application-Id' = [],
	 'Acct-Application-Id' = []}).

-record('diameter_relay_E2E-Sequence', {'AVP' = []}).
...
msg_header(_) ->
    erlang:error({unexpected, diameter_gen_relay}).
...
name2rec('Proxy-Info') -> 'diameter_relay_Proxy-Info';
name2rec('Failed-AVP') -> 'diameter_relay_Failed-AVP';
...
dict() ->
	...
	[{'Disconnect-Cause',
	 ...
	 {'Redirect-Host-Usage',
	  [{0, 'DONT_CACHE'}, {1, 'ALL_SESSION'},
	   {2, 'ALL_REALM'}, {3, 'REALM_AND_APPLICATION'},
	   {4, 'ALL_APPLICATION'}, {5, 'ALL_HOST'},
	   {6, 'ALL_USER'}]},
	 {'Auth-Request-Type',
	  [{1, 'AUTHENTICATE_ONLY'}, {2, 'AUTHORIZE_ONLY'},
	 ...
	 {'Session-Server-Failover',
	  [{0, 'REFUSE_SERVICE'}, {1, 'TRY_AGAIN'},
	   {2, 'ALLOW_SERVICE'}, {3, 'TRY_AGAIN_ALLOW_SERVICE'}]},
	 {'Accounting-Record-Type',
	  [{1, 'EVENT_RECORD'}, {2, 'START_RECORD'},
	   {3, 'INTERIM_RECORD'}, {4, 'STOP_RECORD'}]},
	 {'Accounting-Realtime-Required',
	  [{1, 'DELIVER_AND_GRANT'}, {2, 'GRANT_AND_STORE'},
	   {3, 'GRANT_AND_LOSE'}]}]}]},
     {import_groups,
      [{diameter_gen_base_rfc3588,
	[{'Proxy-Info', 284, [],
	  [{'Proxy-Host'}, {'Proxy-State'}, {'*', ['AVP']}]},
	 {'Failed-AVP', 279, [], [{{1, '*'}, {'AVP'}}]},
	 {'Experimental-Result', 297, [],
	  [{'Vendor-Id'}, {'Experimental-Result-Code'}]},
	 {'Vendor-Specific-Application-Id', 260, [],
	  [{{1, '*'}, {'Vendor-Id'}}, ['Auth-Application-Id'],
	   ['Acct-Application-Id']]},
